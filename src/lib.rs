...
macro_rules! expr {
    ((if $test: tt $then: tt $else: tt)) => {
        $crate::If<$crate::expr!($test), $crate::expr!($then),  $crate::expr!($else)>
    };

    ((cons $a:tt $b:tt)) => {
        $crate::Cons<$crate::expr!($a), $crate::expr!($b)>
    };
    (nil) => {
        $crate::Nil
    };
    ...
    ((lambda ( $( $param:ident )* ) $body:tt)) => {
        $crate::Lambda<$crate::list_t!($( $param ),*), $crate::expr!($body)>
    };

    ((begin $( $e:tt )+)) => {
        $crate::Begin<$crate::list_t!($( $crate::expr!($e)),*)>
    };

    // Support both `(call/ec Tag f)` and `(callec Tag f)` spellings.
    ((call / ec $tag:ident $f:tt)) => {
        $crate::CallEC<$tag, $crate::expr!($f)>
    };
    ...
    ((cdr $a:tt)) => {
        $crate::Cdr<$crate::expr!($a)>
    };

    ((car $a:tt)) => {
        $crate::Car<$crate::expr!($a)>
    };

    ((equalp $a:tt $b:tt)) => {
        $crate::Equalp<$crate::expr!($a), $crate::expr!($b)>
    };
    ((apply $f:tt $args:tt)) => {
        $crate::ApplyExpr<$crate::expr!($f), $crate::expr!($args)>
    };
    ...
    ((+ $a:tt $b:tt)) => {
        $crate::Plus<$crate::expr!($a), $crate::expr!($b)>
    };

    ((* $a:tt $b:tt)) => {
        $crate::Mul<$crate::expr!($a), $crate::expr!($b)>
    };

    ((- $a:tt $b:tt)) => {
        $crate::Sub<$crate::expr!($a), $crate::expr!($b)>
    };

    ((= $a:tt $b:tt)) => {
        $crate::Eq<$crate::expr!($a), $crate::expr!($b)>
    };
    ...
    ((< $a:tt $b:tt)) => {
        $crate::Lt<$crate::expr!($a), $crate::expr!($b)>
    };

    ((> $a:tt $b:tt)) => {
        $crate::Gt<$crate::expr!($a), $crate::expr!($b)>
    };

    ($k: ident = $v:tt) => {
        $crate::SetBang<$k, $crate::expr!($v), $crate::expr!(body)>
    };
